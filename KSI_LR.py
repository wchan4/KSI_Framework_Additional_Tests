...
class LogisticRegression(nn.Module):
    def __init__(self, batch_size, vocab_size, tagset_size):
        ...
        self.linear = nn.Linear(Embeddingsize, tagset_size)
    ...
    def forward(self, vec1, nvec, wiki, simlearning):
        ...
        output = self.linear(thisembeddings)
        if simlearning == 1:
            tag_scores = self.sigmoid(output.detach() + vec3)
        else:
            tag_scores = self.sigmoid(output)
...
model = LogisticRegression(batchsize, len(word_to_ix), len(label_to_ix))
model.cuda()
...
torch.save(basemodel, 'LogisticRegression_model')
...
torch.save(KSImodel, 'KSI_LogisticRegression_model')
...
print ('LogisitcRegression alone:           ')
testmodel(basemodel, 0)
print ('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX')
print ('KSI+LogisticRegression:           ')
